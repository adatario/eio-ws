...
module Websocket_eio = struct
  open Websocket
  module Io = Websocket.Make (Cohttp_eio.Private.IO)
  ...
  let send_frames stream oc =
    ...
    let rec loop () =
      ...
      Buffer.clear buf;
      Io.write_frame_to_buf ~mode:Server buf frame;
  ...
  let read_frames stream ic oc =
    let read_frame = Io.make_read_frame ~mode:Server ic oc in
    let rec loop () =
      let frame = read_frame () in
      Eio.Stream.add stream frame;
      loop ()
    in
    loop ()

  let upgrade_connection ~sw request =
    ...
    let frames_out = Eio.Stream.create 1 in
    let frames_in = Eio.Stream.create 1 in

    let f ic oc =
      Eio.Fiber.fork ~sw @@ fun () ->
      Eio.Fiber.both (send_frames frames_out oc) (read_frames frames_in ic oc)
    in

    (`Expert (resp, f), frames_in, Eio.Stream.add frames_out)
end

let handler ~sw _socket request _body =
  match Http.Request.resource request with
  | "/" -> `Response (Http.Response.make (), Cohttp_eio.Body.of_string text)
  | "/html" ->
      `Response
        ( Http.Response.make
  ...
  | "/ws" ->
      (* TODO: read potential body from flow *)
      let resp, _incoming, _send_frame =
        Websocket_eio.upgrade_connection ~sw request
      in
      resp
  | _ ->
      `Response
...
let main env =
  ...
  and server =
    Cohttp_eio.Server.make_response_action ~callback:(handler ~sw) ()
  in
